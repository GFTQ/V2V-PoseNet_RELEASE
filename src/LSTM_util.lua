    ...
    local function enqueue()
        while idx <= trainSz and threads:acceptsjob() do
            ...
            threads:addjob(
                ...
                inputs = torch.Tensor(seq_length,sz,inputDim,croppedSz,croppedSz,croppedSz):fill(bkgValue)
                heatmaps = torch.CudaTensor(seq_length,sz,jointNum,croppedSz/poolFactor,croppedSz/poolFactor,croppedSz/poolFactor):zero()
                for i, idi in ipairs(indices:totable()) do
                  ...
                  for j = 1, seq_length do 
                    local idj = math.min(trainSz,idi+j-1)
                    --load each data
                    local jointworld = _jointWorld[idj]:clone()
                    local refPt = _RefPt[idj]:clone()
                    local input_name = _allName[idj]
                    local depthimage = load_depthmap(input_name)
                    --voxelizing
                    inputs[j][i] = generate_cubic_input(inputs[j][i][1],depthimage,refPt,newSz,angle,trans)
                    --filename_depth ='/home/gft/handback/handcode/OFF_LSTM_visual/data/NYU/depth' .. string.format('1_%07d',i) .. '.mat'
                    --filename_voxel ='/home/gft/handback/handcode/OFF_LSTM_visual/data/NYU/voxel' .. string.format('1_%07d',i) .. '.mat'
                    --depthimage = depthimage:type('torch.DoubleTensor')
                    --mattorch.save(filename_depth, depthimage)
                    --mattorch.save(filename_voxel, inputs[i])
                    --print (i)
                    --generate 3D heatmap
                    heatmaps[j][i] = generate_heatmap_gt(heatmaps[j][i],jointworld,refPt,newSz,angle,trans)
                  end
